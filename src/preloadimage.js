...
;( function(exports) {
		...
		var preload = function(url, ready, load, error) {
			var __ = this, list = [], intervalId = null, tick = function() {
				var i = 0;
				for (; i < list.length; i++) {
		...
		preload.prototype.circuGet = function(img, ready) {
			...
			var check = function() {
				...
				if (time > 5)
					return;
				window.console && console.log(img, ' try to get img width and height!!');
		...
		exports.preloadimage = function(url, ready, load, error) {
